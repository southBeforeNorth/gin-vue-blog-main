...
const menuOptions = [
  ...
  { text: '印象', icon: 'mdi:calendar-range', path: '/diary' },
...
<template>
  <UDrawer v-model="collapsed" placement="right" :width="250">
    <div class="mx-5">
      ...
      <!-- 登录 -->
      <div>
        <template v-if="userStore.nickname === '管理员'">
          <RouterLink to="/createDiary">
            <div class="m-2 flex items-center p-1" @click="appStore.setCollapsed(false)">
              <Icon icon="mdi:calendar-range" class="text-lg" />
              <span class="ml-5"> 创建日记 </span>
            </div>
          </RouterLink>
        </template>
