...
const onUploadImg = async (files, callback) => {
  try {
    ...
    const urls = results
      .filter(r => r.code === 0)
      .map(r => `${window.location.origin}/${r.data}`)
    
    callback(urls)
    $message.success(`上传成功 ${urls.length} 张图片`)
  } catch (error) {
    $message.error('上传失败')
  }
}
...
//       if (file) {
//         const name = `paste-${Date.now()}.${file.type.split('/')[1]}`
...
const handleDrop = async (event) => {
  ...
  if (imageFiles.length > 0) {
    await uploadImages(imageFiles, '拖拽')
  }
}
...
// 统一上传处理
const uploadImages = async (files, source) => {
  try {
    const results = await Promise.all(files.map(uploadFile))
    const urls = results
      .filter(r => r.code === 0)
    ...
    if (urls.length > 0) {
      // 插入到编辑器
      const images = urls.map(url => `![image](${url})`).join('\n\n')
      formModel.value.content += (formModel.value.content ? '\n\n' : '') + images
      
      $message.success(`${source}上传成功 ${urls.length} 张图片`)
    }
  } catch (error) {
    $message.error(`${source}上传失败`)
  }
}
