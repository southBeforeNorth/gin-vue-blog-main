...
func registerBaseHandler(r *gin.Engine) {
	...
	base.GET("/email/verify", userAuthAPI.VerifyCode)//注册邮箱验证
	base.POST("/send-password-reset-code", userAuthAPI.SendPasswordResetCode)//修改密码发送验证码
	base.POST("/reset-password", userAuthAPI.ResetPassword)//修改密码
}
...
func registerAdminHandler(r *gin.Engine) {
	...
	// 日记模块
	diary := auth.Group("/diary")
	{
		diary.GET("/:id", diaryAPI.GetDetail)                // 日记详情
		diary.GET("/list", diaryAPI.GetListByAdmin)          // 日记列表
		diary.POST("", diaryAPI.SaveOrUpdate)                // 新增/编辑日记
		diary.PUT("/soft-delete", diaryAPI.UpdateSoftDelete) //软删除
		diary.DELETE("", diaryAPI.Delete)                    //物理删除
	}
...
func registerBlogHandler(r *gin.Engine) {
	...
	diary := base.Group("/diary")
	{
		diary.GET("/list", diaryAPI.GetListByFront) // 前台日记列表
	}
	...
	// 鉴定权限
	base.Use(middleware.PermissionCheck())
	{
		base.POST("/diary", diaryAPI.SaveOrUpdate) // 新增/编辑日记
	}
}
