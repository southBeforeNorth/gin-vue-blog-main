...
func (*ipUtil) GetIpAddress(c *gin.Context) (ipAddress string) {
	...
	if strings.HasPrefix(ipAddress, "127.0.0.1") || strings.HasPrefix(ipAddress, "[::1]") {
		ip, err := externalIP()
		if err != nil {
			slog.Error(fmt.Sprintf("GetIpAddress, externalIP, err: %s", err.Error()))
		}
		ipAddress = ip.String()
	}
...
func (*ipUtil) GetIpSource(ipAddress string) string {
	var dbPath = "/gvb/assets/ip2region.xdb" // IP 数据库文件
	// 完全基于文件查询, 每次都读取文件
	...
	if err != nil {
		slog.Error(fmt.Sprintf("failed to create searcher with vector index: %s", err.Error()))
		return ""
	}
